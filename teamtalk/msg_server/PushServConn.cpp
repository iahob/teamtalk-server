...
{
	SPDLOG_ERROR("connect to push server success ");
	m_bOpen = true;
...
void CPushServConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		SPDLOG_ERROR("conn to push server timeout ");
		Close();
	}
}
...
void CPushServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        case CID_OTHER_HEARTBEAT:
            //SPDLOG_ERROR("push server heart beat. ");
            break;
        case CID_OTHER_PUSH_TO_USER_RSP:
            _HandlePushToUserResponse( pPdu );
            break;
        default:
            SPDLOG_ERROR("push server, wrong cmd id={} ", pPdu->GetCommandId());
	}
}
...
void CPushServConn::_HandlePushToUserResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandlePushToUserResponse ");
}
