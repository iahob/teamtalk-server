...
{
	SPDLOG_ERROR("connect to file server success ");
	m_bOpen = true;
...
void CFileServConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		SPDLOG_ERROR("conn to file server timeout ");
		Close();
	}
}
...
void CFileServConn::_HandleFileMsgTransRsp(CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleFileMsgTransRsp, result: {}, from_user_id: {}, to_user_id: {}, file_name: {}, task_id: {}, trans_mode: {}. ", result, from_id, to_id,
...
void CFileServConn::_HandleFileServerIPRsp(CImPdu* pPdu)
{
    ...
    for (uint32_t i = 0; i < ip_addr_cnt ; i++)
    {
        ...
        SPDLOG_ERROR("_HandleFileServerIPRsp -> {} : {} ", ip_addr.ip().c_str(), ip_addr.port());
