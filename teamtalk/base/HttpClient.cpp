...
//    curl_easy_setopt(curl, CURLOPT_POST, 1);
    ...
    if (CURLE_OK != res) {
        SPDLOG_ERROR("curl_easy_perform failed, res={}", res);
        return "";
    }
    ...
    if (!reader.parse(strResp, value)) {
        SPDLOG_ERROR("json parse failed: {}", strResp.c_str());
        return "";
    }
    
    if (value["error_code"].isNull()) {
        SPDLOG_ERROR("no code in response {}", strResp.c_str());
        return "";
    }
    uint32_t nRet = value["error_code"].asUInt();
    if(nRet != 0)
    {
        SPDLOG_ERROR("upload faile:{}", nRet);
        return "";
    }
...
bool CHttpClient::DownloadByteFile(const string &url, AudioMsgInfo* pAudioMsg)
{
    ...
    if(CURLE_OK != res || retcode != 200) {
        SPDLOG_ERROR("curl_easy_perform failed, res={}, ret={}", res, retcode);
    }
    double nLen = 0;
