...
namespace DB_PROXY {
    ...
    //            *pContact = *it;
            ...
            SPDLOG_ERROR("userId= {}, last_time={}, count={}", nUserId, nLastTime, msgResp.contact_session_list_size());
    ...
    void deleteRecentSession(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(IM::BaseDefine::SessionType_IsValid(nType))
            {
                ...
                SPDLOG_ERROR("userId={}, peerId={}, result={}", nUserId, nPeerId, bRet?"success":"failed");
            ...
            {
                SPDLOG_ERROR("invalied session_type. userId={}, peerId={}, seseionType={}", nUserId, nPeerId, nType);
            }
        }
        else{
            SPDLOG_ERROR("parse pb failed");
