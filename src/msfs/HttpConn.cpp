            ...
            if (nFileNameLen <= 255) {
                ...
                if (pPosType != NULL) {
                    ...
                    if (nTypeLen <= 15) {
                        ...
                        szType[nTypeLen] = 0;
                        SPDLOG_ERROR("upload file, file name:{}", szFileName);
                        char szExtend[16];
...
CHttpConn::CHttpConn()
{
    ...
    //SPDLOG_ERROR("CHttpConn, handle={}", m_conn_handle);
}
...
void CHttpConn::OnRead()
{
    ...
    //SPDLOG_ERROR("OnRead, buf_len={}, conn_handle={}", buf_len, m_conn_handle); // for debug
    ...
    if (m_HttpParser.IsReadAll())
    {
        ...
        SPDLOG_ERROR("IP:{} access:{}", m_peer_ip.c_str(), strUrl.c_str());
