...
bool CGroupMessageModel::sendMessage(uint32_t nFromId, uint32_t nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,uint32_t nMsgId, const string& strMsgContent)
{
    ...
    {
        SPDLOG_ERROR("not in the group.fromId={}, groupId={}", nFromId, nGroupId);
    }
    return bRet;
}
...
bool CGroupMessageModel::clearMessageCount(uint32_t nUserId, uint32_t nGroupId)
{
    ...
    if (pCacheConn)
    {
        ...
        if(bRet)
        {
            ...
            if(strReply.empty())
            {
                SPDLOG_ERROR("hmset {} failed !", strUserKey.c_str());
            }
            else
            {
                bRet = true;
        ...
        {
            SPDLOG_ERROR("hgetAll {} failed !", strGroupKey.c_str());
        }
    }
    else
    {
    ...
    if (pDBConn)
    {
        ...
        if (pResultSet)
        {
            ...
            while(pResultSet->Next())
            {
                ...
                {
                    SPDLOG_ERROR("invalid msgType. userId={}, groupId={}, msgType={}", nUserId, nGroupId, nMsgType);
                }
            }
            delete pResultSet;
        }
        ...
        {
            SPDLOG_ERROR("no result set for sql: {}", strSql.c_str());
        }
        pDBManager->RelDBConn(pDBConn);
        if (!lsMsg.empty()) {
...
//                SPDLOG_ERROR("hget %s : count failed !", strGroupKey.c_str());
            ...
            if(nCount > 0)
            {
                ...
                {
                    SPDLOG_ERROR("invalid msgType. userId={}, groupId={}, msgType={}, msgId={}", nUserId, nGroupId, nType, nMsgId);
                }
            }
        }
        pCacheManager->RelCacheConn(pCacheConn);
...
void CGroupMessageModel::getMsgByMsgId(uint32_t nUserId, uint32_t nGroupId, const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg)
{
    if(!lsMsgId.empty())
    {
        ...
        {
            SPDLOG_ERROR("{} is not in group:{}", nUserId, nGroupId);
        }
    }
    else
    {
