...
namespace DB_PROXY {
    ...
    //            *pInfo = *it;
            ...
            SPDLOG_ERROR("userId={}, unreadCnt={}, totalCount={}", nUserId, msgResp.unreadinfo_list_size(), nTotalCnt);
    ...
    void clearUnreadMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            SPDLOG_ERROR("userId={}, peerId={}, type={}", nFromId, nUserId, nSessionType);
        }
        else
        {
            SPDLOG_ERROR("parse pb failed");
    ...
    void setDevicesToken(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (pCacheConn)
            {
                ...
                SPDLOG_ERROR("setDeviceToken. userId={}, deviceToken=%s", nUserId, strToken.c_str());
    ...
    void getDevicesToken(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
                    ...
                    for (auto it=mapTokens.begin(); it!=mapTokens.end(); ++it) {
                        ...
                        if( nPos != string::npos)
                        {
                            ...
                            if(nPos!=string::npos)
                            {
                                ...
                                {
                                    SPDLOG_ERROR("invalid clientType.clientType={}", nClientType);
                                }
                            }
                            else
                            {
            ...
            SPDLOG_ERROR("req devices token.reqCnt={}, resCnt={}", nCnt, msgResp.user_token_info_size());
