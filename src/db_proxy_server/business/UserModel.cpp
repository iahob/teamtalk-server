...
bool CUserModel::updateUser(DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        ...
        if(!bRet)
        {
            SPDLOG_ERROR("updateUser: update failed:{}", strSql.c_str());
        }
        pDBManager->RelDBConn(pDBConn);
    }
    else
    ...
    if(IM::BaseDefine::SessionType_IsValid(nSessionType))
    {
        ...
        if (pCacheConn)
        {
            ...
            if(nSessionType == IM::BaseDefine::SESSION_TYPE_SINGLE)
            {
                ...
                if(!nRet)
                {
                    SPDLOG_ERROR("hdel failed {}->%d", nPeerId, nUserId);
                }
            }
        ...
        SPDLOG_ERROR("invalid sessionType. userId={}, fromId={}, sessionType={}", nUserId, nPeerId, nSessionType);
    }
}
...
void CUserModel::setCallReport(uint32_t nUserId, uint32_t nPeerId, IM::BaseDefine::ClientType nClientType)
{
    ...
    {
        SPDLOG_ERROR("invalid clienttype. userId={}, peerId={}, clientType={}", nUserId, nPeerId, nClientType);
    }
}
