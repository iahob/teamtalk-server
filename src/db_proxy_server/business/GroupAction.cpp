...
namespace DB_PROXY {
    ...
    void createGroup(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(IM::BaseDefine::GroupType_IsValid(nGroupType))
            {
                ...
                SPDLOG_ERROR("createGroup.{} create {}, userCnt={}", nUserId, strGroupName.c_str(), setMember.size());
                ...
                SPDLOG_ERROR("createGroup.{} create {}, userCnt={}, result:{}", nUserId, strGroupName.c_str(), setMember.size(), msgResp.result_code());
            ...
            {
                SPDLOG_ERROR("invalid group type.userId={}, groupType={}, groupName={}", nUserId, nGroupType, strGroupName.c_str());
            }
        }
        else
        {
    ...
    void getNormalGroupList(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            SPDLOG_ERROR("getNormalGroupList. userId={}, count={}", nUserId, msgResp.group_version_list_size());
    ...
    //            *pGroupInfo = *it;
            ...
            SPDLOG_ERROR("userId={}, requestCount={}", nUserId, nGroupCnt);
    ...
    void modifyMember(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (IM::BaseDefine::GroupModifyType_IsValid(nType) &&
                ...
                SPDLOG_ERROR("userId={}, groupId={}, result={}, changeCount:{}, currentCount={}",nUserId, nGroupId,  bRet?0:1, msgResp.chg_user_id_list_size(), msgResp.cur_user_id_list_size());
                msgResp.set_attach_data(msg.attach_data());
                pPduRes->SetPBMsg(&msgResp);
                pPduRes->SetSeqNum(pPdu->GetSeqNum());
                pPduRes->SetServiceId(IM::BaseDefine::SID_GROUP);
            ...
            {
                SPDLOG_ERROR("invalid groupModifyType or groupId. userId={}, groupId={}, groupModifyType={}", nUserId, nGroupId, nType);
            }
    ...
    void setGroupPush(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(CGroupModel::getInstance()->isValidateGroupId(nGroupId))
            {
                ...
                SPDLOG_ERROR("userId={}, groupId={}, result={}", nUserId, nGroupId, msgResp.result_code());
            ...
            {
                SPDLOG_ERROR("Invalid group.userId={}, groupId={}", nUserId, nGroupId);
            }
        }
        else
        {
    ...
    void getGroupPush(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        //            *pStatus = *it;
                ...
                SPDLOG_ERROR("groupId={}, count={}", nGroupId, nUserCnt);
            ...
            {
                SPDLOG_ERROR("Invalid groupId. nGroupId={}", nGroupId);
            }
        }
        else
        {
