...
{
	if (sig_no == SIGTERM) {
		SPDLOG_ERROR("receive SIGTERM, prepare for exit");
        CImPdu cPdu;
...
void CProxyConn::OnRead()
{
    ...
    try {
        ...
        SPDLOG_ERROR("!!!catch exception, err_code={}, err_msg={}, close the connection ",
...
void CProxyConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		SPDLOG_ERROR("proxy connection timeout {}:{}", m_peer_ip.c_str(), m_peer_port);
		Close();
	}
}
...
void CProxyConn::HandlePduBuf(uchar_t* pdu_buf, uint32_t pdu_len)
{
    ...
    if (handler) {
        ...
        SPDLOG_ERROR("no handler for packet type: {}", pPdu->GetCommandId());
    }
}
	...
	while (!s_response_pdu_list.empty()) {
		...
		if (pConn) {
			if (pResp->pPdu) {
				...
				SPDLOG_ERROR("close connection uuid={} by parse pdu error\b", pResp->conn_uuid);
				pConn->Close();
			}
		}
