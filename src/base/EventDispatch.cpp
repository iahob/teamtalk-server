...
CEventDispatch::CEventDispatch()
{
	...
	if (m_kqfd == -1)
	{
        SPDLOG_INFO("kqueue failed");
 	}
#else
	m_epfd = epoll_create(1024);
	if (m_epfd == -1)
	{
		SPDLOG_ERROR("epoll_create failed");
	}
#endif
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			SPDLOG_ERROR("select failed, error code: {}", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			//SPDLOG_ERROR("select return read count={}\n", read_set.fd_count);
		...
		{
			//SPDLOG_ERROR("select return write count={}\n", write_set.fd_count);
		...
		{
			//SPDLOG_ERROR("select return exception count={}\n", excep_set.fd_count);
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
			...
			{
				//SPDLOG_ERROR("OnRead, socket=%d\n", ev_fd);
				pSocket->OnRead();
			}

			if (events[i].filter == EVFILT_WRITE)
			{
				//SPDLOG_ERROR("OnWrite, socket=%d\n", ev_fd);
				pSocket->OnWrite();
			}
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		SPDLOG_ERROR("epoll_ctl() failed, errno={}", errno);
	}
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
            ...
            {
                //SPDLOG_ERROR("On Peer Close, socket=%d, ev_fd);
                pSocket->OnClose();
            }
            #endif
            // Commit End
