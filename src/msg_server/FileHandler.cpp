...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleClientFileRequest, {}->{}, fileName: {}, trans_mode: {}.", from_id, to_id, file_name.c_str(), trans_mode);
    ...
    {
        SPDLOG_ERROR("HandleClientFileRequest, no file server.   ");
...
void CFileHandler::HandleClientFileHasOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleClientFileHasOfflineReq, req_id={}   ", req_user_id);
    ...
    {
        SPDLOG_ERROR("warning no DB connection available ");
...
void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    string file_name = msg.file_name();
    uint32_t file_size = msg.file_size();
    SPDLOG_ERROR("HandleClientFileAddOfflineReq, {}->{}, task_id: {}, file_name: {}, size: {}  ",
...
void CFileHandler::HandleClientFileDelOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    uint32_t to_id = msg.to_user_id();
    string task_id = msg.task_id();
    SPDLOG_ERROR("HandleClientFileDelOfflineReq, {}->{}, task_id={} ", from_id, to_id, task_id.c_str());
...
void CFileHandler::HandleFileHasOfflineRes(CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleFileHasOfflineRes, req_id={}, file_cnt={} ", req_user_id, file_cnt);
    ...
    {
        SPDLOG_ERROR("HandleFileHasOfflineRes, no file server. ");
    }
    if (pConn) {
        pPdu->SetPBMsg(&msg);
        pConn->SendPdu(pPdu);
...
void CFileHandler::HandleFileNotify(CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleFileNotify, from_id: {}, to_id: {}, file_name: {}, task_id: {}, trans_mode: {},offline_ready: {}. ", from_user_id, to_user_id, file_name.c_str(), task_id.c_str(),
